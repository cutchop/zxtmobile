...
<head runat="server">
    <title>电子围栏</title>
    <style type="text/css">
        body {font-size: 12px;color: #666;font-family: Arial,宋体;margin:0px;padding:0px;}
        ...
        *html .div_float_logo{background-image:none;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src="images/first_ico.png", sizingMethod="");}
        ...
        #div_list{ width:180px; border:1px solid #25467d; position:absolute; left:70px; top:80px; z-index:999;}
        #div_list_head{width:180px; height:20px; color:#FFFFFF; line-height:20px; background:#16589a; border-bottom:3px solid #699f19; float:left; position:relative;}
         #div_toolbox{ width:310px; border:1px solid #25467d; position:absolute; right:20px; top:20px; z-index:999;}
        #div_toolbox_head{width:310px; height:20px; color:#FFFFFF; line-height:20px; background:#16589a; border-bottom:3px solid #699f19; float:left; position:relative;}
        ...
        .div_float_content{ background-color:#FFF; padding:10px; line-height:18px; overflow:auto}
    ...
    function listinit(){
        if(fences.length>0){
            for (var i = 0; i < fences.length; i++) {
                $("#listtable").append("<tr><td style=\"width:100px;\"><a href=\"javascript:showfence('"+fences[i].id+"')\">"+fences[i].name+"</a></td><td><a href=\"javascript:editfence('"+fences[i].id+"','"+fences[i].name+"',"+fences[i].type+")\">编辑</a>&nbsp;<a href=\"#\" onclick=\"delfence('"+fences[i].id+"','"+fences[i].name+"',this)\">删除</a></td></tr>");
            }
        }else{
            $("#listtable").append("<tr><td><br /><br />目前没有电子围栏<br />可以点击右上角的按钮添加</td></tr>");
        }
    }
        ...
        for (var i = 0; i < fences.length; i++) {
            if (fences[i].type == "1") {
                mapObj.addOverlays(new MMap.Circle({
                    id: fences[i].id,
                    center: fences[i].center,
                    radius: fences[i].radius,
                    strokeColor: "#0000ff",
                    strokeOpacity: 0.5,
                    strokeWeight: 2,
                    fillColor: "#f5deb3",
    ...
    function delfence(id,name,obj) {
        if(confirm("确定要删除围栏["+name+"]吗？")){
            $.post("fencedel.ashx", {
                id: id
            }, function (result) {
                if (result == "s") {
...
<body onload='mapInit();'>
    ...
    <div id="div_toolbox" style="display:block;">
        <div id="div_toolbox_head">&nbsp;&nbsp;&nbsp;添加围栏</div>
        <div style="height:40px;" class="div_float_content">
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tbody>
                  <tr>
                      <td><input type="button" class="btnbg" value="画圆形围栏" style="width:90px;" onclick="draw(1)" />
                      </td>
                      <td><input type="button" class="btnbg" value="画矩形围栏" style="width:90px;" onclick="draw(2)" />
                      </td>
                      <td><input type="button" class="btnbg" value="画多边形围栏" style="width:90px;" onclick="draw(3)" />
                      </td>
                  </tr>
                </tbody>
                </table>
